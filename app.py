...
def bhmie(x, refrel, nang):
    # This file is converted from mie.m, see http://atol.ucsd.edu/scatlib/index.html
    ...
    for n in range(0, nstop):
        ...
        if (n > 0):
            gsca += ((en - 1.) * (en + 1.) / en) * (
                    real(an1) * real(an) + imag(an1) * imag(an) + real(bn1) * real(bn) + imag(bn1) * imag(bn))

        # *** Now calculate scattering intensity pattern
        #    First do angles from 0 to 90
        pi = 0 + pi1  # 0+pi1 because we want a hard copy of the values
        tau = en * amu * pi - (en + 1.) * pi0
        s1_1 = s1_1 + fn * (an * pi + bn * tau)
        s2_1 = s2_1 + fn * (an * tau + bn * pi)
        ...
        p = -p
        s1_2 = s1_2 + fn * p * (an * pi - bn * tau)
        s2_2 = s2_2 + fn * p * (bn * pi - an * tau)
    ...
    print("S1:", s1, '\nS2', s2, '\nЭффективность затухания: ', qext, '\nЭффективность рассеяния:  ', qsca,
          '\nЭффективность обратного рассеяния:  ', qback, '\nПараметр асимметрии:  ', qsca)
    file = open('test.txt', 'a')
    stroka = '\n\n\nS1: {0}\nS2: {1}\nЭффективность затухания: {2}\nЭффективность рассеяния: {3}\nЭффективность обратного рассеяния: {4}\nПараметр асимметрии: {5}'.format(s1, s2, qext, qsca, qback, gsca)
    file.write(stroka)
    file.close()
    return s1, s2, qext, qsca, qback, gsca


bhmie(0.00000000000783594726192, 1.3330, 90)
